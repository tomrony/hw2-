...
//    	  leaveMessage(key);
   ...
   */
   private void login(String key)
   {
      if (key.equals("#"))
      {
    	  if(accumulatedKeys.contains("2"))
    	  {
    		  accumulatedKeys="";
    		  System.out.println("enter passcod");
    		  Scanner scan = new Scanner(System.in);
    		  String code = scan.nextLine();
         if (currentMailbox.checkPasscode(code))
         {
            state = MAILBOX_MENU;
      ...
      else if (accumulatedKeys.contains("1"))
      {
    	  System.out.println("Leave a message now.");
    	 Message newMessage =new Message( accumulatedKeys);
    	 currentMailbox.addMessage(newMessage);
    	 accumulatedKeys="";
      }
      }
      else
